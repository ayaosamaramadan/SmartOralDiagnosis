...
export default function Alldiseases() {
  return (
    <>
    <div className="mt-8 bg-gradient-to-b from-gray-900 to-gray-950 py-12">
      ...
      <p className="text-lg sm:text-xl text-center mb-10 text-gray-400 leading-relaxed max-w-2xl">
      ...
      {Oralsdata.map((oral) => (
        ...
        <div className="transition-transform duration-300 transform will-change-transform overflow-hidden flex flex-col md:flex-row items-stretch cursor-pointer hover:shadow-xl hover:-translate-y-1 focus-within:ring-2 focus-within:ring-blue-400 rounded-2xl border border-transparent bg-gradient-to-r from-gray-800/60 to-gray-800/40 backdrop-blur-sm">
          ...
          <h2 className="text-2xl sm:text-3xl font-semibold text-white mb-2 transition-colors duration-300 group-hover:text-blue-200">
            {oral.title}
          </h2>
          <p className="text-gray-200 flex-1 transition-colors duration-300 group-hover:text-gray-300 leading-relaxed">
            {oral.description}
          </p>
          </div>
        </div>
