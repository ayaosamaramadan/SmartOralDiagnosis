...
export async function POST(req: Request) {
  try {
    // Accept multiple possible env var names so deployments aren't brittle.
    const apiKey =
    ...
    if (!apiKey) {
      return NextResponse.json(
        {
          message:
            "Missing Gemini API key on server. Set GEMINI_API_KEY (or GOOGLE_API_KEY / GOOGLE_CLOUD_API_KEY) in frontend/.env.local or your environment",
        },
        { status: 500 }
      );
    }
    ...
    if (!res.ok) {
      ...
      // Detect quota / billing / rate-limit related failures and return a clear message
      const quotaRegex = /quota|exceed|exceeded|limit|rate limit|billing|resource_exhausted/i;
      const errorCode = d?.error?.code;
      const isQuotaError =
        statusCode === 429 ||
        statusCode === 403 ||
        quotaRegex.test(errMsg) ||
        (typeof errorCode !== "undefined" && String(errorCode).toLowerCase().includes("resource_exhausted"));

      if (isQuotaError) {
        return NextResponse.json(
          {
            message:
              "Gemini quota exceeded or billing not enabled for this API key. Check your Google Cloud billing / quotas and enable the Generative Language API (see https://ai.google.dev/gemini-api/docs/rate-limits).",
          },
          { status: 429 }
        );
      }
